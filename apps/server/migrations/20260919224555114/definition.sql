...
--
-- MUST be re-appended after every `serverpod create-migration`: the generator
-- rebuilds definition.sql from the models, which cannot express a partial
-- index. Without it the target schema omits the constraint and a repair
-- migration would DROP it, silently removing the DB-level dedupe guarantee.
--
...
--
-- Class RepositoryCredentialRow as table repository_credential
--
