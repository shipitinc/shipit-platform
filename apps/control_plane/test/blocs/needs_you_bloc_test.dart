...
void main() {
  group('NeedsYouBloc', () {
    ...
    test('initial state is empty', () {
      ...
      expect(bloc.state.decisions, isEmpty);
    });

    test('emits loaded state with data on NeedsYouLoaded', () async {
      repository.decisionsResponse = [
        DecisionResponse(
          decisionId: 'dec-001',
          workItemId: 'wi-001',
      ...
      expect(bloc.state.decisions.length, 2);
      expect(bloc.state.decisions.first.id, 'dec-001');
      expect(bloc.state.decisions.first.runTitle, 'Deploy frontend');
      expect(bloc.state.decisions.first.question, 'Approve deployment?');
    });

    test('emits error state on failure', () async {
      repository.throwError = Exception('Service unavailable');

      final bloc = NeedsYouBloc(repository: repository)..add(NeedsYouLoaded());
      await bloc.stream.firstWhere((s) => !s.isLoading);

      expect(bloc.state.errorMessage, isNotNull);
      expect(bloc.state.errorMessage!, contains('Service unavailable'));
    });
  });
}
