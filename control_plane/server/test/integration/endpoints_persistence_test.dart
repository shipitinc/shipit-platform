...
/// sets [WorkItem.state]; the endpoint responses are plain JSON built from the
/// same store reads the processes would make after a restart.
void main() {
    ...
    (sessionBuilder, endpoints) {
      ...
      test(
        ...
        () async {
          ...
          final wire = response['workItem'] as Map<String, dynamic>;
          expect(wire['workItemId'], 'wi-inspect-1');
          expect(wire['state'], WorkItemState.waitingForHumanDecision.wire);
          expect(wire['status'], isNot(contains('taskResults')));

          final history = response['transitionHistory'] as List;
          expect(
            history,
            isNotEmpty,
          ...
          expect(
            (history.map((h) => (h as Map<String, dynamic>)['toState'])).last,
      ...
      test(
        ...
        () async {
          ...
          final wire = decisions.first as Map<String, dynamic>;
          expect(wire['decisionId'], decisionId);
          expect(wire['status'], HumanDecisionStatus.pending.wire);
          expect(wire['workItemId'], 'wi-decisions-1');
        },
      );
      ...
      test('workflow.resolveDecision unlocks the gate via the engine and '
        ...
        final decisionWire = response['decision'] as Map<String, dynamic>;
        ...
        expect(
          (replay['workItem'] as Map<String, dynamic>)['state'],
          WorkItemState.designApproved.wire,
          reason:
      ...
      test(
        ...
        () async {
          ...
          final stateViaEndpoint =
              (inspect1['workItem'] as Map<String, dynamic>)['state'] as String;
          ...
          expect(
            (inspect2['workItem'] as Map<String, dynamic>)['state'],
            stateViaEndpoint,
      ...
      test('endpoints round-trip timestamps in ISO-8601 UTC', () async {
        ...
        final wire = response['workItem'] as Map<String, dynamic>;
        expect(DateTime.parse(wire['createdAt'] as String), isA<DateTime>());
        expect(
          DateTime.parse(wire['createdAt'] as String).isUtc,
          isTrue,
