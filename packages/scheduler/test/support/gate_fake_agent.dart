...
/// worker execution in flight (capacity-1 behaviour) without any timers. When
/// released it writes the reference calculator implementation into the
/// session's working directory (the isolated worktree), so the verifier and
/// git-observed diff capture behave exactly like the worker slice's fake.
///
...
/// and auto-releases so one host can run multiple sequential executions (e.g.
/// a scheduler dispatching Wi-B and then Wi-C on its single worker).
class GateFakeAgentSession implements AgentSession {
  GateFakeAgentSession({
  ...
  // Created once and NEVER replaced: the awaiting/release futures are stable
  // references a test may await before the first execution begins.
  ...
  Future<void> start(AgentSessionConfig config) async {
    ...
    if (_firstExecutionConsumed) {
      // Only the FIRST execution parks on the release gate so a test can
      // observe the in-flight run; every later execution auto-releases and
      // runs straight to completion (the worker slice's un-gated fake).
  ...
  Future<void> close() async {
    // No-op: this session is a reusable scripted runner. A real runtime tears
    // the session down here; our gate simply resets on the next [start].
  }
}
...
class GateFakeAgentAdapter implements AgentAdapter {
  ...
  final GateFakeAgentSession _session;

  @override
  final String providerId = 'fake-runtime';

  @override
  final String displayName = 'Gated Fake Runtime';
