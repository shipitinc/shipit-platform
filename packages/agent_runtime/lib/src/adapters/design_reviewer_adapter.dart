...
class DesignReviewerAdapter implements AgentAdapter {
  DesignReviewerAdapter({AcpTransportFactory? transportFactory})
    : _transportFactory = transportFactory ?? defaultAcpTransport;

  final AcpTransportFactory _transportFactory;

  @override
  final String providerId = 'opencode-design-reviewer';

  @override
  final String displayName = 'OpenCode Design Reviewer';
  ...
  Future<AgentSession> createSession(AgentSessionConfig config) async {
    final session = DesignReviewerSession(
...
class DesignReviewerSession extends OpenCodeSession {
  DesignReviewerSession({
